...
impl InternalContext {
    ...
    fn advance(&mut self, behavior: Behavior, intent: &BehaviorIntentSnapshot) {
        let p = parameters();
...
impl BehaviorDynamicsState {
    pub fn new(seed: u64, intent: BehaviorIntentSnapshot) -> Self {
        let context = InternalContext::default();
        let bucket = context.bucket();
        let (draw, target) = duration_draw(seed, 0, Behavior::Quiet, bucket);
        Self {
            schema_version: 1,
            ...
            elapsed_frames: 0,
            minimum_dwell_frames: parameters().for_behavior(Behavior::Quiet).minimum_frames,
    ...
    pub fn validate(
        ...
        {
            return Err("invalid, missing or inconsistent N4 causal state".into());
        }
        let d = parameters().for_behavior(behavior);
        let (key, target) = duration_draw(
            seed,
        ...
        {
            return Err(
                "N4 duration draw or declared dwell does not match the parameter artifact".into(),
            );
        }
        Ok(())
    }
    ...
    pub fn advance(&mut self, seed: u64, intent: BehaviorIntentSnapshot) -> DynamicsDecision {
        ...
        self.context.advance(self.current_macro_state, &intent);
        self.update_latches(&intent);
        let p = parameters();
        ...
        if let Some((next, reason)) = self.external_candidate(exclude) {
        ...
        let (next, reason) = self.context_candidate(exclude);
    ...
    fn update_latches(&mut self, intent: &BehaviorIntentSnapshot) {
        let p = parameters();
        for (i, value) in [
    ...
    fn external_candidate(&self, exclude: Option<Behavior>) -> Option<(Behavior, Reason)> {
        if self.context.fatigue_q15 >= parameters().fatigue_critical_q15 {
            return None;
        }
    ...
    fn context_candidate(&self, exclude: Option<Behavior>) -> (Behavior, Reason) {
        let p = parameters();
        let c = self.context;
    ...
    fn enter(
        ...
        let (draw, target) = duration_draw(seed, sequence, next, bucket);

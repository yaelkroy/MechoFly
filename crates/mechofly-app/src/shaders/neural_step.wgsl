...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    let neuron_index = id.x;
    if neuron_index >= params.neuron_count {
        return;
    }

    var drive = clamp(stimulus.values[neuron_index], -8192, 8192);
    let start = offsets.values[neuron_index];
    let end = offsets.values[neuron_index + 1u];
    ...
    let noise_hash = mix32(
        params.seed_folded
            ...
            ^ (neuron_index * 0x9e3779b9u),
    );
    drive = clamp(drive + i32(noise_hash & 0x1ffu) - 256, -65536, 65536);

    let candidate = (previous.values[neuron_index] * 13 + drive * 3) / 16;
    if candidate > 8000 {
        output.values[neuron_index].activation = clamp(candidate - 10000, -32768, 32767);
        output.values[neuron_index].spike = 1u;
    } else {
        output.values[neuron_index].activation = clamp(candidate, -32768, 32767);
        output.values[neuron_index].spike = 0u;
    }
}
